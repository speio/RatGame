import time, webbrowser

def yes_open():
	print "*nibble nibble tear tear*"
	print "That's you opening the package with your face becasuse you're a rat."

	print """Whoa! That's nuts, I never would have guessed 
	you'd get such a delicious package.
 	Too bad you don't have any milk or jelly to go with that"""

	todo = "nothing"
	repeating = "no"
	while todo == "nothing":
		if repeating == "yes":
			print "So, what's inside"		
		else:
			print "What do you think is inside?"
	
		inside = raw_input("> ")
	
		if "peanut" in list(inside.split()) or 'oreos' in list(inside.split()):
			todo = "continue"
	
		else:
			print "errr"
			todo = "nothing"	
			repeating = "yes"
					
			print "I'll give you a hint" 
			time.sleep(1)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep( 0.201)

			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep( 0.201)

			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			
			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
            8    8      -8-- ----:8    8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			
			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
           8    8       -8-- ----:8   8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
           8    8       -8-- ----:8   8   8
           -:::88        8:::::: -8   88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
           8    8       -8-- ----:8   8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
            8    8      -8-- ----:8    8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""							

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
            -:::88       8:::::: -8    88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""										

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""									

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""									

			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""									
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8    88::-
                  -8:    8:    :- 8  :8-    
                   -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
           8    8       -8-- ----:8   8   8
            -:::88       8:::::: -8   88::-
                  -8:    8:    :- 8 :8-    
                  -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8    88::-
                  -8:    8:    :- 8  :8-    
                   -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)						
			time.sleep( 0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""									
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8    88::-
                  -8:    8:    :- 8  :8-    
                   -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
           :---8:      8 -- 8 -- 88   :8--:    
           8    8       -8-- ----:8   8   8
            -:::88       8:::::: -8   88::-
                  -8:    8:    :- 8 :8-    
                  -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
            :---8:     8 -- 8 -- 88    :8--:    
            8    8      -8-- ----:8    8   8
             -:::88      8:::::: -8    88::-
                  -8:    8:    :- 8  :8-    
                   -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)
			print """     
                           :---:            
                           8   8:
                          8-    8:           
                        ::8: ::::8           
                       8 ** 8 ** 88          
             :---8:    8 -- 8 -- 88     :8--:    
             8    8     -8-- ----:8     8   8
             -:::88      8:::::: -8     88::-
                  -8:    8:    :- 8   :8-    
                    -----8 ----  :8----      
                         8       8        
                         8      8-           
                         8     8            
                         888888  It's Peanut             
                         8     8             
                        8-     -8      Butter Jelly
                     ::::8       8::               Time!         
                   8-    -8    8-   -8        
                   -------      ------        
							"""
			time.sleep(0.201)						

							
	print "Close enough!"
	time.sleep(0.7)
	print "Peanut Butter and Oreos in the box, yo \n don't let that go to waste!"


def poem():
	print """


                                                    
                                                    
 ___ .-.     .--.    ___  ___    .--.    ___ .-.    
(   )   \   /    \  (   )(   )  /    \  (   )   \   
 |  .-. .  |  .-. ;  | |  | |  |  .-. ;  | ' .-. ;  
 | |  | |  |  | | |  | |  | |  |  | | |  |  / (___) 
 | |  | |  |  |/  |  | |  | |  |  |/  |  | |        
 | |  | |  |  ' _.'  | |  | |  |  ' _.'  | |        
 | |  | |  |  .'.-.  ' '  ; '  |  .'.-.  | |        
 | |  | |  '  `-' /   \ `' /   '  `-' /  | |        
(___)(___)  `.__.'     '_.'     `.__.'  (___)       
                                                    
                                                    


"""
	time.sleep(.8)
	print """

                   ___      
                  (   )     
  .--.     .---.   | |_     
 /    \   / .-, \ (   __)   
|  .-. ; (__) ; |  | |      
|  | | |   .'`  |  | | ___  
|  |/  |  / .'| |  | |(   ) 
|  ' _.' | /  | |  | | | |  
|  .'.-. ; |  ; |  | ' | |  
'  `-' / ' `-'  |  ' `-' ;  
 `.__.'  `.__.'_.   `.__.   
                            
"""
	time.sleep(.9)
	print """               
      
                                    ___   ___                                         ___  
                                   (   ) (   )                                       (   ) 
  .--.      .---.   ___ .-.      .-.| |   | |.-.     .--.     .---.   ___ .-.      .-.| |  
 /    \    / .-, \ (   )   \    /   \ |   | /   \   /    \   / .-, \ (   )   \    /   \ |  
|  .-. ;  (__) ; |  | ' .-. ;  |  .-. |   |  .-. | |  .-. ; (__) ; |  | ' .-. ;  |  .-. |  
|  |(___)   .'`  |  |  / (___) | |  | |   | |  | | | |  | |   .'`  |  |  / (___) | |  | |  
|  |       / .'| |  | |        | |  | |   | |  | | | |  | |  / .'| |  | |        | |  | |  
|  | ___  | /  | |  | |        | |  | |   | |  | | | |  | | | /  | |  | |        | |  | |  
|  '(   ) ; |  ; |  | |        | '  | |   | '  | | | '  | | ; |  ; |  | |        | '  | |  
'  `-' |  ' `-'  |  | |        ' `-'  /   ' `-' ;  '  `-' / ' `-'  |  | |        ' `-'  /  
 `.__,'   `.__.'_. (___)        `.__,'     `.__.    `.__.'  `.__.'_. (___)        `.__,'   
                                                                                           
    """
	time.sleep(1)
	print "by Phil Hahn"

	time.sleep(1)
	print """\"
Never Eat Cardboard.
It's Not Good For You.
I mean Sometimes When You Open A Package Of Food,
Cardboard From Its Box Could Fall Into It,
But You Should Pick It Out.
And Don't Give Me That Tired Argument About
Needing Fiber In Your Diet.
	"""
	time.sleep(6)
	print """
That's Pathetic.
That's Not The Same Kind of Fiber.
Besides, Cardboard Doesn't Taste That Good.
Well...
	"""
	time.sleep(4)
	print """
I suppose You Could Put Something on it Like
Salt or Ketchup or Something.
Or You Could Cook Something and put Some
Cardboard in it and then it might taste Good.
But You Should Still Never Eat Cardboard.
Not even if You Trip and Fall on a Cardboard Box
And Your Mouth Falls On The Edge Of The Box.
	"""
	time.sleep(5)
	print """
Poster Board Is In The Same Category!
It Is Still a Type of Cardboard
And You Shouldn't eat it either.
After You Finish Your Presentation,
Throw The Posterboard Away!
Don't Eat It.
	"""
	time.sleep(4)
	print """
And Don't Get Me Started on Eating
Homework Assignments.
But I'm Still on Cardboard.
Don't Even Eat Cardboard If Someone Is
Opening a Box Above You With a Knife
Shedding Small Pieces, Some Of Which Fall
Into Your Mouth. You Don't Have To Eat Them!
You Can Spit Them Out!
	"""
	time.sleep(5.5)
	print """
Eating Cardboard Also Makes You Look Silly.
Could You Imagine Seeing Someone Eating
A Buick! How Silly! You Look Just as
Silly Eating Cardboard.
	"""
	time.sleep(5)
	print """
I Love You.
	"""
	time.sleep(2.5)
	print """
And When You Eat Cardboard there is
even a chance you could cut your mouth.
Some of those edges can give you nasty paper cuts.
Paper cuts Would Be real painful on your mouth.
Especially on your tongue.
Now I know that your mouth heals
Fast, But Why Go Through it?
There Are So Many More Things You
Could Eat that are safer.
	"""
	time.sleep(6)
	print """
What if You Were Eating Cardboard and
Got a paper cut under your tongue and
Cut a vein or artery or something and
Bled to death?
Is It really Worth it?
Think of your family before you eat Cardboard.
	"""
	time.sleep(4.5)
	print """

You Also Don't Know Where That Cardboard Has Been!
What if it's dirty!
Maybe the last guy to handle that Box
Didn't wash his hands after he went
To the bathroom.
"""
	time.sleep(4)
	print """
I guess what I'm really trying to say
	"""
	time.sleep(2)
	print """
Is that the internet is a good way
To buy airline tickets.
\"
"""	
	time.sleep(2.5)
	
	while True:
		print "Buy airline tickets? \n (y/n)"
		response = raw_input(" >")
		
		if response == 'y':
			url = "https://www.google.de/flights/#search;f=RAT;t=;"
			webbrowser.open_new(url)
			break
		elif response == 'n':
			print "You don't understand poetry."
			time.sleep(1)
			print  "-10 xp"
			time.sleep(1)
			break
		else:
			print "This is serious"
			time.sleep(1)
			
		
	print "It's a real tear jerker I know,"
	
	print "So if you'd still like to eat it I won't stop you."
	decide = raw_input("Eat the box? (y/n) \n > ")
	while True:
		if decide == '1':
			return "dead box"
		elif decide == 'n':
			print "Well, I know it's hard but that's probably the right choice"
			return "bella both"
		else:
			print "type better"

#decides what to do with the contents of the box, returns a variable "output" which has
#the function to call in the main script(dead or bella), and the result of the decision made about
#the contents of the box or the reason for death
def eating(eat, filtered_eat):
	decision = "none"
	#response as filtred and interpreted by intersect_check function
	response = filtered_eat
	#raw response from user input
	raw = eat
	while decision != "continue":
			
		#eating the peanut butter
		if response == 'peanut butter' or response == 'both':
			decision = "continue"
			output = ("dead", "poison", "continue")
	
		#eating the box
		elif response == 'box':
			print "Look"
			time.sleep(1)
			print "I know you're a rat"
			time.sleep(1)
			print "So eating cardboard is like...totally normal"
			time.sleep(1)
			print "But don't you think the alternative options here are better?"
			time.sleep(.5)
			print "Let me first share with you a beautiful poem on the subject by Phil Hahn:"
			time.sleep(1.5)
			to_do = poem()
			if to_do == "bella both":
				decision = "continue"
				output = ("bella", "both", "continue")
			else:
				decision = "continue"
				output = ("dead", "box", "continue")
			
		#eating only the oreos	
		elif response == 'oreos':
			print "Good choice, oreos are delicious all on their own."
			time.sleep(1)
			print "You're all full of oreos now. Guess you'll just save the peanut butter."
			print "*scoops peanut butter into pocket for later*"
			decision = "continue"
			output = ("bella", "peanut", "continue")

		#use of double negative response
		elif response == 'double_negative':
			print "..."
			time.sleep(1)
			print "Don't be a dick"
			decision = "continue"
			output = ('','','')	
				
		#unspecific
		elif response == 'unspecific':
			print "\" %s \" ???" % raw
			print "Eat what? The box? \n Don't be so unspecific."
			decision = 'continue'
			output = ('','','')
				
		elif response == 'neither':
			print "I guess you're not eating either? \n what a shame."
			while save != 'y' or save != 'n':
				save = raw_input("Should you save them for later? \n (y/n) >")
				if save == "y":
					decision = "continue"
					output = ("bella", "both", "continue")
				elif save == 'n':
					time.sleep(2)
					print "That damn candle!" 
					time.sleep(1)
					decision = 'continue'
					output = ("dead", "fire", "continue")
				else:
					print "Yeah, \n \"%s \" doesn't work here" % save
	
		elif response == 'bad input':
			print "Not sure what you were trying to say, give it another go"
			decision = 'continue'
			output = ('','','')
		
		else:
			print "Else reached inside eating function, meaning intersect_check() returned nonsense"
			exit(1)
	return output

